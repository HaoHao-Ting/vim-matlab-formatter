...
'''
    ...
    https://github.com/affenwiesel/matlab-formatter-vscode
    Copyright(C) 2019-2021 Benjamin "Mogli" Mann

    I made a simple modification to make it applicable to Vim
...
class Formatter:
    ...
    ctrl_1line = re.compile(
        r'(^|\s*)(if|while|for|try)(\W\S.*\W)(end|endif|endwhile|endfor)(\s+\S.*|\s*$)')
    ctrlstart = re.compile(
        r'(^|\s*)(function|if|while|for|parfor|try|classdef|methods|properties|events|arguments|enumeration)\s*(\W\S.*|\s*$)')
    ctrl_ignore = re.compile(r'(^|\s*)(import|clear|clearvars)(.*$)')
    ctrlstart_2 = re.compile(r'(^|\s*)(switch)\s*(\W\S.*|\s*$)')
    ctrlcont = re.compile(
        r'(^|\s*)(elseif|else|case|otherwise|catch)\s*(\W\S.*|\s*$)')
    ctrlend = re.compile(
...
def main():
    ...
    fileName = r'E:\LocalGitea\matlab_Tools\matlab-formatter-vscode-master\formatter\test.m'
    start = 1
    end = 45
    formatter = Formatter(indent, sep)
    formatter.formatFile(fileName, start, end)

    formatter.formatSeq(['a+b','a-2'])
